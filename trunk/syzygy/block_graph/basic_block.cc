...
BasicBlock::BasicBlBasicBlockSubGraph* subgraph,
  ...
  id_(idt_(1),
      type_(type),
      offset_(kNoOffset),
      is_padding_(false  DCHECK(subgraph_ != NULL);e) {
}
...
BasicCodeBlock::BasicCodeBlBasicBlockSubGraph* subgraph,
                               const base::StringPiece& name,
                               BlockId id)
    : BasicBlock(subgraph, name, idname, BASIC_CODE_BLOCK) {
}
...
BasicDataBlock::BasicDataBlBasicBlockSubGraph* subgraph,
                               ock(const base::StringPiece& name,
                           BlockId idname,
                               const uint8* data,
                               Size size)
    : BasicBlsubgraph, ock(naidname, BasicBlock::BASIC_DATA_BLOCK),
      size_(size),
      data_(data) {
  DCHECK(data != NULL);
  DCHECK_NE(0u, size);
...
bool BasicDataBlock::IsValid() const {
  return true;BasicEndBlock::BasicEndBlock(BasicBlockSubGraph* subgraph,
                             BlockId id)
    : BasicBlock(subgraph, kEnd, id, BasicBlock::BASIC_END_BLOCK) {
}

BasicEndBlock* BasicEndDataBlock::Cast(BasicBlock* basic_block) {
  if (basic_block == NULL)
    return NULL;
  if (basic_block->type() == BasicBlock::BAEND_BLOCK)
    return static_cast<BasicEndBlock*>(basic_block);
  return NULL;
}

const BasicEndBlock* BasicEndDataBlock::Cast(const BasicBlock* basic_block) {
  if (basic_block == NULL)
    return NULL;
  if (basic_block->type() == BasicBlock::BAEND_BLOCK)
...
bool BasicEndDataBlock::IsValid() const {
  return true;
}
